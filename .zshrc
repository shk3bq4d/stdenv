...
if ! is_antigen; then
    plugins=(git zsh-autosuggestions history-substring-search vi-mode-mr z kubectl systemd zsh-syntax-highlighting) # zsh-syntax-highlighting must be the last
...
hash -v _z_std_add &>/dev/null && _z_std_add | while read i; do z --add $i; done
