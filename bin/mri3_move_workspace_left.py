...
def go(args=[]):
    ...
    if focused_index == 0:
        logger.info('Returning as already absolute left')
        return
    left_workspace = all_workspaces[focused_index - 1]
    left_number,    left_leftover    = re.match(r'(\d+)(.*)', left_workspace.name   ).groups()
    ...
    logger.info('left_workspace name %s', left_workspace.name)
    logger.info('all_workspaces %s', list(map(lambda x: x.name, all_workspaces)))
    print((left_number, left_leftover))
    print((focused_number, focused_leftover))
    i3.command('rename workspace "{}" to "{}{}"'.format(left_workspace.name,    focused_number, left_leftover))
    i3.command('rename workspace "{}" to "{}{}"'.format(focused_workspace.name, left_number,    focused_leftover))
