...
## dumps the items of the passed ZABBIX TEMPLATE
##
...
def print_items(oH, key, title):
    iA = oH.get(key, [])
    if len(iA) > 0:
        print(title)
        for iH in iA:
        #   pprint(iH)
            print("{key:<80s} {name}".format(**iH))
...
tA = yH.get('templates', [])
tH = tA[0]
print_items(tH, "items", "=== normal items")

dA = tH.get('discovery_rules', [])
for dH in dA:
#   pprint(dH)
    print_items(dH, "item_prototypes", "\n=== disco {key} {name}".format(**dH))
