...
docker run -it --rm --entrypoint="" mr/testimage:version2 bash
docker run -it --rm --entrypoint="" mr/testimage:version2 tail -f /dev/null
...
sudo docker image list --format "table {{.Repository}}:{{.Tag}}"
...
# https://docs.docker.com/desktop/setup/allow-list/ # squid
