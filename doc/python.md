...
pprint(re.search('(?P<first>\S+) (?P<second>\S+)', 'bonjour madame').groupdict()) # {'second': 'madame', 'first': 'bonjour'} group in regular expressinos regexp
...
re.I re.IGNORECASE
re.L re.LOCALE
re.M re.MULTILINE ^ $ not only match beginning and end of input
re.U re.UNICODE
...
import json; json.load(sys.stdin) # parse json
